...
## Button position invariant (user, again 2026-09-23)
The Open anchor is the LAST child of its card — after the seller verbatim block and ☎ contact (never before notes). Verified: regressed 49/54 to mid-card positions on 2026-09-23 by a bulk language pass that re-wrote card markup by index. Root cause generalized: good changes keep getting reverted when a later bulk edit runs against a stale state or re-orders via in-loop slicing. Anti-revert protocol (mandatory for this project and every other project's bulk edit): re-verify ALL known invariants after ANY bulk edit (button labels, button position, parity, price bands, div/h4 balance) — never only the pass's own target. A change only counts as committed once a post-edit audit passes on the FULL invariant set, with git push.
